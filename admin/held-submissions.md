...
| Sonaopus | sonaopus.com | 2026-09-09 | praise.ayo.boyinde@gmail.com | **Vendor replied 2026-09-12** (msg 195). Confirmed: sole proprietor Praise Boyinde, no registered entity yet; local-device storage accurate (browser LocalStorage keys `sonaopus_hr`/`_ea`/`_pm`/`_re`/`_role`, no `fetch`/XHR, no analytics or telemetry scripts); no URL-passing (`claude.ai/new`, `chatgpt.com/`, `gemini.google.com/` with no query params, Gemini via clipboard); $99 confirmed current on all five Gumroad listings. $59 traced to a **stale third-party mirror** — `devadex.com/software/gumroad%3Asonaopusaiworkflow-ai-hr-management-workflow` mirrors retired account `sonaopusaiworkflow.gumroad.com/l/eiddl` (now 404, still indexed in search). Recorded as stale mirror, not a vendor contradiction. **Still open:** public Privacy Policy + ToS URLs (blocker), controller jurisdiction + service address, standalone legal-review disclosure, refund terms in ToS. `sonaopus.com` has **no MX** — the Gmail address is the only reachable contact; do not ask for a domain address. |
