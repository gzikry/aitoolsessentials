...
| Sonaopus | sonaopus.com | 2026-09-09 | praise.ayo.boyinde@gmail.com | **PUBLISHED 2026-09-12** (commit `328c5223`) after the vendor cleared verification. Vendor published `/privacy` + `/terms` on 2026-09-12, naming operator Praise Boyinde (sole proprietor), governing law Nigeria, user-retained output ownership, no-refunds position, and the subprocessors that actually touch data (Netlify, Gumroad). Local-storage claim re-verified from shipped JS: keys `sonaopus_hr`/`_ea`/`_pm`/`_re`/`_role`, zero `fetch`/XHR/sendBeacon, no third-party scripts, no URL-passing (`claude.ai/new`, `chatgpt.com/`, `gemini.google.com/` carry no query params). $99 confirmed on all five Gumroad products. $59 traced to a **stale third-party mirror** — `devadex.com` mirrors retired account `sonaopusaiworkflow.gumroad.com` (now 404, still indexed). **Residual caveats recorded in `unresolved_claims`, not blockers:** homepage FAQ still says "Nothing stored" where the Privacy Policy says browser local storage (vendor agreed in writing to correct it); no export for local data; legal-review disclosure lives in workflow prompts only; no service address published; no MX on the domain so the Gmail address is the only reachable contact; stale mirror still advertises $59. Publication reply sent 2026-09-12. Re-checked the promised **remedy** on 2026-09-13: `/` still ships "What you type stays yours / Everything runs in your browser" alongside the FAQ line "Nothing stored. Nothing leaves your own AI conversation" — the specific line the vendor agreed to align is still live. Cosmetic, already recorded as a caveat; no re-hold, no third email. Re-check once more on the next review sweep. |
