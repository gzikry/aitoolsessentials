...
- **SaaSHub (free queue).** Real form: `https://www.saashub.com/services/submit` — `/submit` is a different product (their 107-directory list). Flow: URL → name/tagline/categories/competitors/email → Free button (not $75 Priority+) → Alpine.js competitor cards (`div.boxed`, click the card not the outbound `<a>`) → category checkboxes → confirmation `https://www.saashub.com/aitoolsessentials/added`. Categories used: AI, AI Tools, Software Directory. Competitors: FutureTools.io, OpenTools, AIToolIntel. Contact: aitoolsessentials@gmail.com.
  **Rejected / still pending 2026-09-13.** SaaSHub asked for competitors, ≥3 categories, logo and pricing. Fixed the same day from the site's own facts — see the 2026-09-13 row in `backlink-outreach-2026-09.md`. The listing was sitting in the wrong categories (`Finance > Subscriptions > Subscription Tracking`) because the submission-time category checkboxes did not stick; the current set is `AI Tools, AI Tools Directory, Software Directory`. Approval up to 32 days; re-review in about a week.
  **How to edit the listing:** SaaSHub emails one-time `https://www.saashub.com/token-login/<token>?redirect_to=/manage/aitoolsessentials/...` links (competitors / questions / edit). Opening one signs you in as the product owner — no account or password needed — and unlocks `/manage/aitoolsessentials/{edit,pricing,categories,alternatives,questions,images,features}`. That is the correct route (SaaSHub's mail says explicitly: do not send details by email). The logged-out `/product-changes/aitoolsessentials/new` form is approval-queued and does a poor job on categories.
  **Cloudflare note:** `www.saashub.com` serves a JS challenge to headless/plain HTTP clients (403 or "Just a moment..."). A headed persistent Playwright profile (`launch_persistent_context` with `--disable-blink-features=AutomationControlled`) passes on the first load and can drive the management pages.
  **Not available:** logo/identity verification is still George-only — SaaSHub needs an email on the product domain, and it wants a "Verify" step for full ownership. `contact@aitoolsessentials.com` is a Porkbun forwarder, not an IMAP mailbox, so that verification link cannot be completed by the agent.
