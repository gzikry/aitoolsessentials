...
`data/pricing_snapshots.json` as it stands today (`updated: 2026-09-25`).
...
1. **The live set is unchanged.** All 47 carried URLs re-verified live today (HTTP 200, request body
   ...
   the third consecutive day, and both reply routes were re-resolved off the live pages again today.
2. **The pair range held for the third consecutive day.** `scripts/extract_monthly_annual_pairs.py`
...
3. **The new window produced nothing, for the sixth run in seven.** 128 slugs since yesterday's mark,
   12 carrying an AI token. **Zero carry an AI token plus a real spend token** — the third consecutive
   run with none. The single strict regex hit was `Seattle` matching `seat`. All 12 AI-token slugs were
   fetched and read in full; none contains a price, seat, licence or spend ask.
4. **Two rows crossed the cold line, and they cost opposite amounts.** Enterprise AI Leaders — Value
   Creation went 10d → 11d: pitched 2026-09-15, no reply in ten days, so it leaves the live set as an
   unanswered send. EdTech — LMS audit went 11d → 12d and was already recorded as skipped, so it cost
   nothing. The distinction is the point: only one of those is a live prospect lost.

**The shadow-AI draft is now 8 days old and two days from the cold line.** It has been written and
unsent for seven consecutive runs.
...
## 1. Shadow AI spend — employees paying out of pocket ◀ SEND THIS FIRST, day 8 unsent

**Source:** https://www.sourcee.app/journo-request/fulltime-employees-shadow-ai-use-and-paying-outofpocket
**Posted:** 2026-09-17 11:40 UTC — **8 days** (page badge today: "Posted in last 7 days")
...
**https://www.raconteur.net/contributors/simon-chandler** returns HTTP 200 and still carries
`data-part1="simon.chandler" data-part2="raconteur" data-part3="net"`, which the site's JS assembles at
runtime. Control on the same run: `/contributors/tom-dennis` (HTTP 200) carries `tom.dennis`
`raconteur` `net`. The byline URL cited on 2026-09-19 (`/author/simon-chandler/`) still returns **HTTP
404** (re-checked today) — it is dead and must not be cited.
**George's lane. Not automatable — it is a plain email send, not a platform reply, but the address is a
reconstructed byline address rather than one published in the request body, so it is sent by hand.**
...
(`updated: 2026-09-25`); 40 tools publish a non-zero monthly price, so the 31 is measured against that
population. **19 same-tier monthly-vs-annual pairs across 14 tools, 1.11x–2.53x, median 1.25x** from
`data/monthly_annual_pairs.json`, re-derived today (built 2026-09-25); Browse AI $48 vs $19 is the
...
**Posted:** 2026-09-17 16:05 UTC — **7 days** (page badge today: "Posted in last 7 days")
...
rework it. **This is now 7 days old and the October issue window is closing — send it today or drop it.**
...
recomputed today from `data/pricing_snapshots.json` (`updated: 2026-09-25`); 19 tiers, 1.11x–2.53x from
...
| Request | Why not |
|---|---|
| **Anthropic — customer service (13d, medium-high, COLD)** | Crossed the >10-day line on 2026-09-23. Draft finished and unsent since 2026-09-19 at `pitch-drafts-2026-09-22.md` §2, route Signal `hliwrites.99` (still verbatim on the live page, re-checked today). Send late or mark skipped — six runs unsent already. |
| **FinOps — agentic AI cost overruns (18d, high, COLD)** | The highest-relevance request we have never answered. Draft since `pitch-drafts-2026-09-17.md` §1, route LinkedIn DM to `linkedin.com/in/niloy-ghosh`. Send it late or drop it — do not redraft. |
| **NYT Wirecutter — AI note-takers across professions (1d)** | The closest near-miss in the window. Note-takers are a category we track, but the ask is practitioners' on-the-record experiences of using them, and the page publishes **no email, no DM handle, no route at all**. Recorded, not drafted — same standing problem as the Inc. and Forbes founders requests. |
| New this window: 12 AI-token slugs (Seattle K-2 screen time, agent-forensics series, advertising to AI agents, interior designers, AI builders in Africa, Wirecutter note-takers, women in AI, decentralized AI, workers whose jobs shifted, sales-enablement onboarding, Chinese suppliers for Spain, AI self-diagnosis case study) | All twelve were fetched and read in full. Every one is a different sector's adoption story, a personal-testimony call or a booking solicitation; **none contains a price, seat, licence or spend ask.** No real AI+spend slug appeared in the window at all — the third run running. |
| Enterprise AI Leaders — value creation (11d, crossed cold today) | Pitched 2026-09-15 to `hello@foxandspindle.com`. Ten days of silence. It leaves the live set as an unanswered send, not an unsent draft — the opposite cost to the EdTech crossing. |
| UK conservative-commentary Substack launch / licensed-psychologist FiveM / UK menopause supplements / DINCs / tungsten pricing / unsigned musicians (all 1-2d) | The spend-token slugs from the window. Each matched a spend token (`budget`, `spending`, `pricing`, `paid`) on a subject that is not software spend — the same class of false positive as `Seattle`/`seat`. Recorded, excluded. |
| BBC — London businesses that dropped AI hiring tools (4d) | Wants a London employer's own decision; no cost angle in the body; address redacted with no route resolved. |
| Forbes — founders cutting AI use (6d, medium-low) | Body is explicit: "Please only answer as a comment on this post. Do not email or DM me." Wants a named founder's own rationale on the record. |
| Enterprise AI Leaders — agent sprawl (20d, high) / Scientists paying for AI (20d, high) | Both genuinely on-beat, both well past the cold line and unpitched. The cost of the cold queue, recorded so the loss is visible. |
| EdTech LMS audit (12d) | Already recorded as skipped on 2026-09-15 — never sendable, so the crossing cost nothing again. |
| Individual contributors managing agents (15d) | Pitched 2026-09-18 to Rani@Sherwood.news. No reply in seven days. |
| Amplemarket Growth/Elite (18d, high) | Pitched 2026-09-18; Jan Suski replied 2026-09-18 20:39Z, answered 21:29Z. Live thread — do not re-pitch. **A correction is still owed on the wrong 1.21x floor — see the ledger's `figures_disclosure`.** |
...
**Mailbox checked 2026-09-25** (`himalaya`): **no reply to any pitch.** INBOX top is still a Google
...
send (ten days) or the 2026-09-18 Sherwood send (seven days).**
...
whose single accessible source has now produced **no core-beat request on six of the last seven runs and
no real AI+spend slug for three consecutive runs**. Endpoint re-confirmed live this run (HTTP 401,
