...
sm_path = "/tmp/sitemap_0915.xml"
if not os.path.exists(sm_path) or os.path.getsize(sm_path) < 1000000:
    r = subprocess.run(["curl","-sL","-m","180","-A",UA,"-o",sm_path,
                        "https://www.sourcee.app/sitemap-journo-requests.xml"], capture_output=True, text=True)
    print("curl rc", r.returncode, r.stderr[:200])

s = open(sm_path, errors="ignore").read()
...
# window: since 2026-09-02 (same as prior run), plus tight window since 09-14 run marker
SINCE = "2026-09-02"
recent = {k:v for k,v in slugs.items() if v >= SINCE}
print("slugs lastmod >=", SINCE, len(recent))
tight = {k:v for k,v in slugs.items() if v >= "2026-09-14T09:00:00"}
print("slugs lastmod >= 2026-09-14T09:00 (since last run)", len(tight))
...
json.dump({"sitemap_bytes": len(s), "pairs": len(pairs), "unique_slugs": len(slugs),
           "newest": lms[-1], "since_0902": len(recent), "tight_since_0914": len(tight),
           "ai": ai, "strict": strict, "strict_ai": strict_ai, "third_ai": third_ai},
          open(f"{D}/_scan_0915.json","w"), indent=1)

print("\n--- NEW since last run marker (tight) ---")
for k,v in sorted(tight.items(), key=lambda x:-x[1] if False else 0):
    pass
